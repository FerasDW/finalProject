...
export const getBotResponse = (userMessage) => {
    ...
    // Greeting responses
    if (containsAny(lowerCaseMessage, ["hello", "hi", "hey", "good morning", "good afternoon", "good evening"])) {
        const greetings = [
    ...
    if (containsAny(lowerCaseMessage, ["how are you", "how do you do", "what's up", "whats up"])) {
        const responses = [
    ...
    // Goodbye responses
    if (containsAny(lowerCaseMessage, ["bye", "goodbye", "see you", "farewell", "take care"])) {
        const goodbyes = [
    ...
    // Help responses
    if (containsAny(lowerCaseMessage, ["help", "assist", "support", "what can you do"])) {
    ...
    if (containsAny(lowerCaseMessage, ["course", "classes", "subjects", "enrollment", "enroll"])) {
        if (lowerCaseMessage.includes("how") || lowerCaseMessage.includes("enroll")) {
    ...
    if (containsAny(lowerCaseMessage, ["assignment", "homework", "task", "due date", "deadline"])) {
        if (lowerCaseMessage.includes("submit") || lowerCaseMessage.includes("upload")) {
    ...
    if (containsAny(lowerCaseMessage, ["grade", "score", "marks", "result", "performance"])) {
    ...
    if (containsAny(lowerCaseMessage, ["profile", "account", "personal info", "edit profile"])) {
    ...
    // Thank you responses
    if (containsAny(lowerCaseMessage, ["thank you", "thanks", "appreciate", "grateful"])) {
        const thanks = [
    ...
    // Navigation help
    if (containsAny(lowerCaseMessage, ["navigate", "find", "where", "location", "menu"])) {
    ...
    // Technical issues
    if (containsAny(lowerCaseMessage, ["problem", "issue", "error", "bug", "not working", "broken"])) {
    ...
    if (containsAny(lowerCaseMessage, ["great", "awesome", "excellent", "perfect", "amazing"])) {
