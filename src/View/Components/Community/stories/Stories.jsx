...
const Stories = () => {
  ...
  // Find the index of current user in groupedStories
  ...
  const handleOpenCurrentUserStory = () => {
    if (currentUserStories.length > 0) {
      setOpenUserIndex(currentUserIndex);
      setCurrentStoryIndex(0);
      setStoryProgress(0);
    } else {
      handleAddStory();
  ...
  const handleNextUser = () => {
    if (openUserIndex < groupedStories.length - 1) {
      setOpenUserIndex(openUserIndex + 1);
      setCurrentStoryIndex(0);
      setStoryProgress(0);
    } else {
  ...
  const handleNextStory = () => {
    const currentUser = groupedStories[openUserIndex];
  ...
  const handlePrevStory = () => {
    if (currentStoryIndex > 0) {
      ...
      setOpenUserIndex(openUserIndex - 1);
      const prevUser = groupedStories[openUserIndex - 1];
  ...
  return (
    <div className="stories">
      ...
      {groupedStories
        .filter((user) => user.userId !== currentUser.id) // Don't show current user again in the list
        .map((user, index) => (
          <div
            className="story"
            key={user.userId}
            onClick={() =>
              handleOpenStory(
                user.userId < currentUser.id ? index : index + 1 // Adjust index to account for current user position
              )
            }
          >
      ...
      {groupedStories.length <= 1 && ( // <=1 because current user is always included
        <div className="no-stories">
          <p>No stories from friends to show</p>
        </div>
      )}

      {openUserIndex !== null && groupedStories[openUserIndex] && (
        <div className="story-overlay">
          <div className="story-header">
            <div className="progress-bars">
              {groupedStories[openUserIndex].stories.map((_, index) => (
                <div
                  key={index}
            ...
            <div className="story-info">
              <img
                src={groupedStories[openUserIndex].profilePic}
                alt="Profile"
                className="story-profile-pic"
                onError={(e) => {
                  e.target.src = groupedStories[openUserIndex].stories[0].img;
                }}
              />
              <span className="story-username">
                {groupedStories[openUserIndex].name}
              </span>
              <span className="story-time">
                {groupedStories[openUserIndex].stories[currentStoryIndex]
                  .time || "2h ago"}
              </span>
            </div>
          ...
          <div className="overlay-content">
            ...
            {(openUserIndex < groupedStories.length - 1 ||
              currentStoryIndex <
                groupedStories[openUserIndex].stories.length - 1) && (
